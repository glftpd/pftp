...
// draw speed
        ...
        mvwaddstr(window, 0, width - strlen(speed_string) - 8,
                  speed_string);
//wnoutrefresh(window);
        update_panels();
        doupdate();
...
/*                for (m = strlen(this->view_buffer), c = this->view_buffer;
   ...
   *c = '.';
    ...
    mvwaddnstr(this->window_log, this->terminal_max_y - 1, 0,
               "ESC to close - CURSOR and PGUP/DN to navigate",
               this->terminal_max_x);
...
void CDisplayHandler::PageMoveLog(bool dir_up)
{
    ...
    if (dir_up) {
        if ((this->log_start + this->terminal_max_y - 2) < (LOG_LINES - 1)) {
            ...
            if ((this->log_start + this->terminal_max_y - 2) >=
                (LOG_LINES - 1))
                this->log_start =
                    (LOG_LINES - 1) - this->terminal_max_y + 2;
        }
    } else {
        if (this->log_start > 0) {
            redraw = TRUE;
...
//            mvwaddnstr(this->window_log, n, 0, this->log[index],
//                       this->terminal_max_x);
...
char *CDisplayHandler::GetFilelistString(FILELIST * filelist, int magic,
                                         int width, bool * is_marked,
                                         int format, int soffset)
{
    FILELIST *fl_temp = filelist;
    int n = 0;
    ...
    if (fl_temp) {
        *is_marked = fl_temp->is_marked;
        ...
        if (strlen(fl_temp->name) < (unsigned)soffset) {
	   strcpy(temp_name,"");
	} else {
           strncpy(temp_name, fl_temp->name + soffset, PATHLEN);
        }
        ...
        if (format == 0) {
            ...
            sprintf(string, "%-*s %s %s", width - 18, temp_name, owner,
                    group);
        }
    } else {
...
void CDisplayHandler::UpdateTabBar(bool dont_remove)
{
    ...
    int remove_magic = -1, draw_magic = -1, remove_ypos = -1, draw_ypos =
        -1, width_remove = -1, width_draw = -1;
...
// remove old bar
    if (filelist_remove && !dont_remove) {
        ...
        if (format_remove == 0) {
            mvwaddch(window_remove, remove_ypos, width_remove - 8,
                     ACS_VLINE);
//              mvwaddch(window_remove, remove_ypos, width_remove - 12, ACS_VLINE);
        } else if (format_remove == 3) {
            mvwaddch(window_remove, remove_ypos, width_remove - 8,
                     ACS_VLINE);
            mvwaddch(window_remove, remove_ypos, width_remove - 17,
                     ACS_VLINE);
        } else if (format_remove == 2) {
            mvwaddch(window_remove, remove_ypos, width_remove - 12,
                     ACS_VLINE);
            mvwaddch(window_remove, remove_ypos, width_remove - 23,
                     ACS_VLINE);
        }
...
// remove in every case
        wattrset(window, COLOR_PAIR(STYLE_NORMAL) | A_NORMAL);
        for (n = 0; n < 22; n++)
            mvwaddch(window, 0, this->window_left_width - 23 + n,
                     ACS_HLINE);
/*              for(n = 0; n < 7; n++)
...
// draw on display
            mvwaddstr(window, 0,
                      this->window_left_width - strlen(this->temp_string) -
                      1, this->temp_string);
        }
    } else {
...
// display cwd
    ...
    if (strlen(cwd) <= (unsigned) (width - 2))
        mvwaddstr(window, this->terminal_max_y - this->status_win_size - 3,
                  1, cwd);
    else
        mvwaddstr(window, this->terminal_max_y - this->status_win_size - 3,
                                           ...
                                           bool redraw_everytime)
{
    ...
    int n, width, height =
        this->terminal_max_y - this->status_win_size - 4, old_ypos, *ypos,
...
// we wiped some info, redraw
        ...
        if (cwd) {
            ...
            if (strlen(cwd) <= (unsigned) width)
                mvwaddstr(this->window_tab,
                          this->terminal_max_y - this->status_win_size - 3,
                          1, cwd);
            else
                mvwaddstr(this->window_tab,
                          this->terminal_max_y - this->status_win_size - 3,
        ...
        if (busy) {
            mvwaddstr(this->window_tab, 0, width + 2 - strlen(busy) - 1,
                      busy);
...
void CDisplayHandler::UpdateFilelistPageMove(bool dir_up)
{
    ...
    int *ypos, entries, *magic, jump =
        (this->terminal_max_y - this->status_win_size - 4) / 2;
...
// no segf check
    ...
    if (fl_temp == NULL)
//like jumping in empty filelist
        return;
//      assert(fl_temp!=NULL);
...
// we wiped some information, redraw...
    if (cwd) {
        ...
        if (strlen(cwd) <= (unsigned) width)
            mvwaddstr(window,
                      this->terminal_max_y - this->status_win_size - 3, 1,
                      cwd);
        else
            mvwaddstr(window,
                      this->terminal_max_y - this->status_win_size - 3, 1,
...
void CDisplayHandler::UpdateServerFilelist(CServer * server,
                                           WINDOW * window)
{
...
void CDisplayHandler::DisplayStatusLog(void)
{
    ...
    int soffset, length, loffset, indent;
...
//      wbkgdset(this->window_status, ' ');
	...
	// this->statuslog[] isnt initialized.. 
	if (this->statuslog[this->status_line + n] != NULL) {
	   // string length is less then the terminal width
	   length = strlen(this->statuslog[this->status_line + n]);	   
	   if (length <= this->terminal_max_x) {
               mvwaddnstr(this->window_status, ypos, 0,
                   this->statuslog[this->status_line + n],
                   this->terminal_max_x);
               ypos--;
	   } else {
               ypos = ypos - ((length - this->terminal_max_x) / (this->terminal_max_x - 6))
                 - (((length - this->terminal_max_x) % (this->terminal_max_x - 6)) ? 1 : 0);
               soffset = loffset = indent = 0;
               while (length > 0) {
                     mvwaddnstr(this->window_status, ypos + loffset, indent,
                           this->statuslog[this->status_line + n] + soffset,
                           this->terminal_max_x - indent);
		     soffset = soffset + this->terminal_max_x - indent;
		     length = length - this->terminal_max_x + indent;
		     if (indent == 0)
                        indent = 6;
		     loffset++;
		}
