...
extern struct _KEYS key_list[127];
//extern bool is_away;
extern bool is_afkish;
...
void CKeyHandler::PostToDisplay(int msg, int extended, int function_key) {
    display->PostMessage(msg, extended, function_key);
}
...
void CKeyHandler::Loop(void) {
    ...
    do {
        ...
        if (is_afkish && key_list[keycode].function_code  == 98) {
        	display->PostStatusLine("Bailing out on all idle sites", TRUE);
        	is_afkish = FALSE;
		    SERVERLIST *sv_temp = global_server;
		    while (sv_temp) {
		    	if (sv_temp->server->GetAfk()) {
		    		//disconnect the site here
		    		sv_temp->server->kill_me=TRUE;
				//PostToDisplay(MSG_KEY_EXTENDED, 0, 98);
		    	}
		        sv_temp = sv_temp->next;
		    }
        	
        }
//        if (is_away && keycode != -1 && key_list[keycode].function_code  != 98) {
//          is_away = FALSE;  
//        }
        if (is_afkish && keycode != -1 && key_list[keycode].function_code  != 98) {
          is_afkish = FALSE;  
		    SERVERLIST *sv_temp = global_server;
		    while (sv_temp) {
		    	sv_temp->server->SetAfk(FALSE);
		        sv_temp = sv_temp->next;
		    }
        ...
        switch (keycode) {
            ...
            case 27: // bah, ncurses owns me
                ...
                //debuglog("FUNCTIONHANDLER ESC - keycode: %d - function: %d", keycode, key_list[keycode].esc_function_code);
                if (key_list[keycode].esc_function_code == 0)
                    PostToDisplay(MSG_KEY_ESCAPE, keycode, key_list[keycode].esc_function_code);
                else
                    PostToDisplay(MSG_KEY_EXTENDED, keycode, key_list[keycode].esc_function_code);
            ...
            */
            ...
            default:
                // keymap handling
                //debuglog("FUNCTIONHANDLER - keycode: %d - function: %d", keycode, key_list[keycode].function_code);
                //if ((key_list[keycode].function_code) > 0)                
                PostToDisplay(MSG_KEY_EXTENDED, keycode, key_list[keycode].function_code);
                //else
                // default keycode handling
    ...
    display->PostStatusLine("shutting down pftp ...", TRUE);
